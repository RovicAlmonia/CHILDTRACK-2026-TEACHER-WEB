...
export default function Layout() {
  ...
  const [font,        setFont]        = useState<'nunito' | 'dmsans'>(() =>
    (localStorage.getItem('ct-font') as 'nunito' | 'dmsans') ?? 'nunito'
  );
  ...
  const toggleFont = () => {
    const next: 'nunito' | 'dmsans' = font === 'nunito' ? 'dmsans' : 'nunito';
    setFont(next);
        ...
        <div className={rootClass}>
          ...
          <aside className={`lt-side${open ? '' : ' lt-mini'}`}>
            ...
            <nav className="lt-nav">
              ...
              {dark && (
                <button
                  className={`lt-nav-btn${superDark ? ' lt-active' : ''}`}
