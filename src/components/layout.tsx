...
const NAV = [
  { label: 'Dashboard',     path: '/',              icon: '📊', countKey: 'dashboard' },
  { label: 'Attendance',    path: '/attendance',    icon: '📋', countKey: 'attendance' },
  { label: 'Absences',      path: '/absences',      icon: '📅', countKey: 'absences' },
  { label: 'Dropouts',      path: '/dropouts',      icon: '🚪', countKey: 'dropouts' },
  { label: 'Guardians',     path: '/guardians',     icon: '🛡️', countKey: 'guardians' },
  { label: 'Students',      path: '/students',      icon: '🎒', countKey: 'students' },
  { label: 'Notifications', path: '/notifications', icon: '🔔', countKey: 'notifications' },
];
...
async function apiFetch(path: string, opts: RequestInit = {}) {
  const token = localStorage.getItem('ct_token'); // matches AuthContext key
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .lt-root { display: flex; height: 100vh; overflow: hidden; font-family: 'Nunito', sans-serif; -webkit-font-smoothing: antialiased; }
  ...
  .lt-mini .lt-nav-btn { justify-content: center; padding: 10px 0; }
  ...
  .lt-nav-icon { font-size: 1rem; flex-shrink: 0; width: 20px; text-align: center; }
  ...
  .pm-tab {
    padding: 7px 16px; border-radius: 8px 8px 0 0; border: none; cursor: pointer;
  ...
  /* Photo area */
  .pm-photo-area {
    display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 20px 24px 0;
  }
  .pm-photo-ring {
    width: 90px; height: 90px; border-radius: 50%; overflow: hidden;
    display: flex; align-items: center; justify-content: center;
    font-size: 2rem; font-weight: 800; color: #fff; position: relative;
  ...
  .pm-photo-overlay {
    ...
    opacity: 0; transition: opacity 0.15s; font-size: 1.4rem;
  }
  ...
  .pm-input {
    padding: 8px 11px; border-radius: 8px; border: 1.5px solid;
    font-size: 0.83rem; font-family: 'Nunito', sans-serif; font-weight: 600;
...
function ProfileModal({ teacher, dark, onClose, onSaved }: ProfileModalProps) {
  ...
  const [photo, setPhoto] = useState<string>(teacher?.photo_base64 ?? '');
  ...
  const saveProfile = async () => {
    ...
    try {
      const data = await apiFetch(`/teachers/${teacher.id}`, {
        method: 'PUT',
  ...
  return (
    <div className={`pm-overlay ${themeClass}`} onClick={handleBackdrop}>
      <div className={`pm-modal ${themeClass}`}>
        ...
        <div className="pm-tabs">
          <button
            className={`pm-tab${tab === 'profile' ? ' pm-tab-active' : ''}`}
          ...
          <>
            ...
            <div className="pm-photo-area">
              <div className="pm-photo-ring" onClick={() => fileRef.current?.click()}>
                ...
                <div className="pm-photo-overlay">📷</div>
              </div>
            ...
            <div className="pm-form">
              <div className="pm-row">
                <div className="pm-field">
                  ...
                  <input className="pm-input" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
                </div>
                <div className="pm-field">
                  <label className="pm-label">Username</label>
                  <input className="pm-input" value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value }))} />
                </div>
              </div>
              <div className="pm-row">
                <div className="pm-field">
                  <label className="pm-label">Age</label>
                  <input className="pm-input" type="number" min="18" max="99" value={form.age} onChange={e => setForm(f => ({ ...f, age: e.target.value }))} />
                </div>
                <div className="pm-field">
              ...
              <div className="pm-row">
                <div className="pm-field">
                  <label className="pm-label">Section / Class</label>
                  <input className="pm-input" value={form.section} onChange={e => setForm(f => ({ ...f, section: e.target.value }))} />
                </div>
                <div className="pm-field">
                  <label className="pm-label">Contact</label>
                  <input className="pm-input" value={form.contact} onChange={e => setForm(f => ({ ...f, contact: e.target.value }))} />
                </div>
              </div>
              <div className="pm-field">
                <label className="pm-label">Address</label>
                <input className="pm-input" value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} />
              </div>
              ...
              <div className="pm-footer">
                ...
                <button className="pm-btn pm-btn-save" onClick={saveProfile} disabled={saving}>
        ...
        {tab === 'password' && (
          <div className="pm-form">
            <div className="pm-field">
              ...
              <input className="pm-input" type="password" value={pw.current} onChange={e => setPw(p => ({ ...p, current: e.target.value }))} placeholder="Enter current password" />
            </div>
            <div className="pm-field">
              <label className="pm-label">New Password</label>
              <input className="pm-input" type="password" value={pw.next} onChange={e => setPw(p => ({ ...p, next: e.target.value }))} placeholder="At least 6 characters" />
              <span className="pm-pw-hint">Must be at least 6 characters.</span>
            </div>
            <div className="pm-field">
              <label className="pm-label">Confirm New Password</label>
              <input className="pm-input" type="password" value={pw.confirm} onChange={e => setPw(p => ({ ...p, confirm: e.target.value }))} placeholder="Repeat new password" />
            </div>
            ...
            <div className="pm-footer">
              <button className="pm-btn pm-btn-cancel" onClick={onClose}>Cancel</button>
              <button className="pm-btn pm-btn-save" onClick={savePassword} disabled={saving}>
...
export default function Layout() {
  ...
  // Per-page new-data counters  (wire these to real API calls as needed)
    ...
    const fetchCount = () => {
      attendanceAPI.getAll(today)
        .then((r: any) => {
          ...
          const attended   = records.filter((x: any) => x.status !== 'Dropped Out').length;
          const absences   = records.filter((x: any) => x.status === 'Absent').length;
          const dropouts   = records.filter((x: any) => x.status === 'Dropped Out').length;
 
          setNotifCount(attended);
 
          // Update per-nav badges — adjust logic to match what "new" means for your app
          setNavCounts({
            attendance:    attended,
            absences:      absences,
            dropouts:      dropouts,
  ...
  const colorMode = useMemo(() => ({ toggleColorMode: toggleDark, mode: dark ? 'dark' as const : 'light' as const }), [dark]);
 
  const muiTheme = useMemo(() => createTheme({
    palette: {
      mode: dark ? 'dark' : 'light',
      primary: { main: dark ? '#38c586' : '#2d5016' },
      background: { default: dark ? '#0f172a' : '#f0f2f5', paper: dark ? '#1e293b' : '#ffffff' },
  ...
  return (
    <ColorModeContext.Provider value={colorMode}>
      <ThemeProvider theme={muiTheme}>
        <CssBaseline />
        <style>{CSS}</style>
        <div className={`lt-root ${dark ? 'lt-dark' : 'lt-light'}`}>
 
          {/* ═══ SIDEBAR ═══ */}
          <aside className={`lt-side${open ? '' : ' lt-mini'}`}>
            <div className="lt-brand">
              <div className="lt-logo-wrap">
                <img src={logoSrc} alt="CHILDTrack" className="lt-logo-img"
            ...
            <nav className="lt-nav">
              ...
              {NAV.map(item => {
                ...
                // Don't show badge on the currently active page (user already sees it)
                const showBadge = count > 0 && !isActive(item.path);
                return (
                  <button
                    key={item.path}
                    className={`lt-nav-btn${isActive(item.path) ? ' lt-active' : ''}`}
                  ...
                  >
                    ...
                    {showBadge && (
                      <span className="lt-notif-badge">{count > 99 ? '99+' : count}</span>
                    )}
                  </button>
                );
              })}
              ...
              <button className="lt-nav-btn" onClick={toggleDark} title={dark ? 'Light mode' : 'Dark mode'}>
                <span className="lt-nav-icon">{dark ? '☀️' : '🌙'}</span>
            ...
            <div className="lt-footer">
              ...
              <button className="lt-logout-btn" onClick={logout} title="Log out">
                <span className="lt-nav-icon">🚪</span>
          ...
          <div className="lt-main">
            <div className="lt-topbar">
              <div className="lt-topbar-left">
                <button className="lt-menu-btn" onClick={() => setOpen(p => !p)}
                ...
                {teacher && (
                  <>
                    ...
                    <div style={{ textAlign: 'left' }}>
                      ...
                      {teacher.section && (
                        <div className="lt-greeting-sub">{teacher.section}</div>
                      )}
                    </div>
                  </>
                )}
 
                <div className="lt-date-chip">📅 {shortDate}</div>
              </div>
            </div>
