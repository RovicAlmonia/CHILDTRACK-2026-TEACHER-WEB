...
export default function NotificationBell({ dark }) {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (attRes.status === 'fulfilled') {
                const todayRecs = (attRes.value.data || []).filter((r) => r.date === today && r.status !== 'Dropped Out');
            ...
            if (evtRes.status === 'fulfilled') {
                ...
                const upcoming = (evtRes.value.data || []).filter((e) => {
    ...
    return (<div style={c.wrap} ref={dropRef}>
      {/* ── Bell button ── */}
      <button style={c.btn} onClick={() => { setOpen(p => !p); if (!open)
        load(); }} title="Notifications">
        🔔
        {unread > 0 && (<span style={c.badge}>{unread > 99 ? '99+' : unread}</span>)}
      </button>

      {/* ── Dropdown ── */}
      {open && (<div style={c.drop}>
          {/* Header */}
          <div style={c.header}>
            <span style={c.headerTitle}>
              Notifications {unread > 0 && `(${unread})`}
            </span>
            {unread > 0 && (<button style={c.markAllBtn} onClick={markAllRead}>
                Mark all read
              </button>)}
          </div>
          ...
          {/* List */}
          <div style={c.scroll}>
            {loading && items.length === 0 ? (<div style={c.empty}>Loading…</div>) : items.length === 0 ? (<div style={c.empty}>
                <div style={{ fontSize: '2rem', marginBottom: 8 }}>🔕</div>
                No notifications yet
              </div>) : (items.slice(0, 30).map(item => (<div key={item.id} style={c.item(item.isRead)} onClick={() => handleItemClick(item)} onMouseEnter={e => {
          ...
          {/* Footer */}
          {items.length > 0 && (<div style={c.footer}>
              <button style={c.seeAll} onClick={() => { setOpen(false); navigate('/notifications'); }}>
