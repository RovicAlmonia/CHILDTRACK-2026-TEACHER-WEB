...
import { Box, Grid, Card, CardContent, Typography, Chip, Skeleton, Avatar, Divider, Button, Dialog, DialogTitle, DialogContent, DialogActions, TextField, MenuItem, Stack, IconButton, Tooltip, } from '@mui/material';
...
const isPresent = (s) => norm(s) === 'present';
const isAbsent = (s) => norm(s) === 'absent';
const isLate = (s) => norm(s) === 'late';
const isDropOff = (s) => norm(s) === 'dropoff';
const isPickUp = (s) => norm(s) === 'pickup';
...
function ScheduleDialog({ open, onClose, onSaved, teacherId, teacherName, editing }) {
    ...
    return (<Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: '18px' } }}>
      <DialogTitle sx={{ fontWeight: 800, fontSize: '1.1rem' }}>
        {editing ? 'Edit Class' : 'Add Class Schedule'}
      </DialogTitle>
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 0.5 }}>
          <TextField label="Subject" value={form.subject} size="small" fullWidth onChange={e => set('subject', e.target.value)}/>
          <TextField select label="Day" value={form.day_of_week} size="small" fullWidth onChange={e => set('day_of_week', e.target.value)}>
            {DAYS.map(d => <MenuItem key={d} value={d}>{d}</MenuItem>)}
          </TextField>
          <Box sx={{ display: 'flex', gap: 1 }}>
            <TextField label="Start time" type="time" value={form.start_time} size="small" fullWidth slotProps={{ inputLabel: { shrink: true } }} onChange={e => set('start_time', e.target.value)}/>
            <TextField label="End time" type="time" value={form.end_time} size="small" fullWidth slotProps={{ inputLabel: { shrink: true } }} onChange={e => set('end_time', e.target.value)}/>
          </Box>
          <TextField label="Room (optional)" value={form.room} size="small" fullWidth onChange={e => set('room', e.target.value)}/>
...
function DeleteDialog({ open, onClose, onConfirm, subject, deleting }) {
    return (<Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: '18px' } }}>
      ...
      <DialogActions sx={{ px: 3, pb: 2.5 }}>
        ...
        <Button onClick={onConfirm} disabled={deleting} variant="contained" color="error" sx={{ borderRadius: '10px', fontWeight: 700 }}>
...
export default function DashboardPage() {
    ...
    useEffect(() => { if (teacher?.id)
        loadSchedules(); }, [loadSchedules, teacher?.id]);
    const openAdd = () => { setEditingItem(null); setDialogOpen(true); };
    const openEdit = (s) => { setEditingItem(s); setDialogOpen(true); };
    const openDelete = (s) => { setDeletingItem(s); setDeleteOpen(true); };
    ...
    return (<Box>
      ...
      <Card sx={{
        ...
        <CardContent sx={{ p: 3 }}>
          ...
          {schedLoading ? (<Grid container spacing={2}>
              {[...Array(3)].map((_, i) => (<Grid key={i} size={{ xs: 12, sm: 6, md: 4 }}>
                  <Skeleton variant="rectangular" height={120} sx={{ borderRadius: 2 }}/>
                </Grid>))}
            </Grid>) : activeDays.length === 0 ? (<Box sx={{ textAlign: 'center', py: 6 }}>
              ...
              {activeDays.map(day => (<Grid key={day} size={{ xs: 12, sm: 6, md: 4 }}>
                  <Box sx={{
                    ...
                    {/* Class rows */}
                    <Box>
                      {byDay[day].map((s, i) => (<Box key={s.id}>
                          <Box sx={{
                        ...
                        '&:hover': { color: mono ? '#ffffff' : DAY_COLOR[day] } }}>
                              ...
                              <Tooltip title="Delete">
                                <IconButton size="small" onClick={() => openDelete(s)} sx={{ color: 'text.secondary',
                        '&:hover': { color: '#e63946' } }}>
      ...
      {/* Add / Edit dialog */}
      <ScheduleDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSaved={loadSchedules} teacherId={teacher.id} teacherName={teacher?.name ?? ''} editing={editingItem}/>

      {/* Delete confirm dialog */}
      <DeleteDialog open={deleteOpen} onClose={() => setDeleteOpen(false)} onConfirm={confirmDelete} subject={deletingItem?.subject ?? ''} deleting={deleting}/>
    </Box>);
}