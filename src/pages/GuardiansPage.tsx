...
import {
  ...
  InputAdornment, Avatar, Tooltip, IconButton, ToggleButtonGroup, ToggleButton,
} from '@mui/material';
...
/* ─── Photo fetcher ─────────────────────────────────────────────────── */
async function fetchGuardianPhoto(
  guardianName: string,
  studentName: string,
  ...
  for (const name of [guardianName, studentName]) {
    ...
    try {
      ...
      const scan = (data as any[]).find(p =>
        p.status?.toLowerCase().replace(/[-_\s]/g, '') === 'guardianregistration'
      );
      if (!scan?.photo_path) continue;
...
function GuardianAvatar({
  guardianName, studentName, photoCache, onPhotoClick,
}: {
  guardianName: string; studentName: string;
  ...
  if (photo && !imgErr) {
    return (
      <Avatar src={photo} alt={guardianName}
        sx={{ ...baseSx, bgcolor: 'transparent', cursor: 'pointer',
          transition: 'transform 0.15s, box-shadow 0.15s',
          '&:hover': { transform: 'scale(1.12)', boxShadow: '0 0 0 2px #6366f1' },
        }}
        onClick={() => onPhotoClick(guardianName, photo)}
        onError={() => setImgErr(true)}
      />
    );
  }
  return (
    <Avatar sx={{ ...baseSx, bgcolor: 'rgba(99,102,241,0.15)', color: '#6366f1' }}>
...
function StatusChip({ status }: { status?: string }) {
  ...
  return (
    <Chip
      icon={<Box sx={{ color: cfg.color, display: 'flex', ml: '6px !important' }}>{cfg.icon}</Box>}
...
export default function GuardiansPage() {
  ...
  const save = async () => {
    if (!form.name.trim())         { setError('Guardian name is required.'); return; }
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        ...
        <Button
          variant="contained" startIcon={<AddIcon />} onClick={() => { setForm({ ...EMPTY }); setError(''); setDialogOpen(true); }}
      ...
      <Alert
        ...
        severity="info"
        sx={{ mb: 2.5, borderRadius: '12px', fontSize: '0.83rem' }}
      >
        ...
        <CardContent sx={{ p: 2 }}>
          <Grid container spacing={2} sx={{ alignItems: 'center' }}>
            <Grid size={{ xs: 12, sm: 5 }}>
              <TextField
                placeholder="Search by name, student, contact…"
          ...
          <Box sx={{ overflowX: 'auto' }}>
            {loading ? (
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    {['Guardian', 'Age', 'Relationship', 'Contact', 'Student', 'Address', 'Parent Decision'].map(h => (
                  ...
                  {displayed.length === 0 ? (
                    ...
                    >
                      ...
                      <TableCell>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                          <GuardianAvatar
                            guardianName={r.name} studentName={r.student_name}
                      ...
                      <TableCell>
                        {r.relationship
                          ? <Chip label={r.relationship} size="small"
                              sx={{ bgcolor: 'rgba(99,102,241,0.12)', color: '#6366f1',
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
          ...
          <Button variant="contained" onClick={save}
            disabled={saving || !form.name.trim() || !form.student_name.trim()}
            startIcon={saving ? <CircularProgress size={16} color="inherit" /> : <SaveIcon />}
            sx={{ px: 3, background: 'linear-gradient(135deg, #6366f1, #4f46e5)', '&:hover': { background: 'linear-gradient(135deg, #4f46e5, #4338ca)' } }}>
            Save Guardian
          </Button>
        </DialogActions>
      </Dialog>
      ...
      {/* Photo dialog */}
      <Dialog open={photoDialog !== null} onClose={() => setPhotoDialog(null)} maxWidth="sm" fullWidth
        ...
        <DialogContent sx={{ p: 0 }}>
          {photoDialog && (
            <Box component="img" src={photoDialog.url} alt={photoDialog.name}
