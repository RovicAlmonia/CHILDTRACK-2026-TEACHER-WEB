...
import { Box, Grid, Card, CardContent, Typography, TextField, Button, MenuItem, Table, TableHead, TableRow, TableCell, TableBody, Chip, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Alert, CircularProgress, Tooltip, Skeleton, InputAdornment, Avatar, Divider, } from '@mui/material';
...
const isDropOff = (s) => normalizeStatus(s) === 'dropoff';
const isPickUp = (s) => normalizeStatus(s) === 'pickup';
const isAbsent = (s) => normalizeStatus(s) === 'absent';
const isPresent = (s) => normalizeStatus(s) === 'present';
const isLate = (s) => normalizeStatus(s) === 'late';
const chipColor = (s) => {
...
function StudentAvatar({ studentName, dark, photoCache, onPhotoClick }) {
...
export default function AttendancePage() {
    ...
    const handleExportCSV = () => {
        ...
        if (dateKey)
            data = data.filter(r => r.date === dateKey);
        if (csvStatusFilter) {
            data = data.filter(r => {
        ...
        if (dateKey && csvStatusFilter)
            filename = `Attendance_${csvStatusFilter}_${dateKey}.csv`;
        else if (dateKey)
            filename = `Attendance_${dateKey}.csv`;
        else if (csvStatusFilter)
            filename = `Attendance_${csvStatusFilter}_${now}.csv`;
    ...
    const handleGenerateSF2 = async () => {
        if (!sf2TemplateFile) {
            setSf2Err('Please upload an SF2 template file first.');
            return;
        }
        setSf2Err('');
        setSf2Msg('');
        ...
        try {
            ...
            setSf2TemplateFile(null);
            if (sf2FileRef.current)
                sf2FileRef.current.value = '';
        }
        catch (err) {
    ...
    return (<Box>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
        {statCards.map(s => (<Card key={s.label} sx={{
                flex: '1 1 0',
                minWidth: 130,
                borderRadius: '14px',
                background: s.bg,
            ...
            <CardContent sx={{
              ...
              {loading ? (<Skeleton variant="circular" width={46} height={46}/>) : (<Box sx={{
                    width: 46,
                    height: 46,
                    borderRadius: '50%',
        ...
        <CardContent sx={{ p: 2 }}>
          <Grid container spacing={2} sx={{ alignItems: 'center' }}>
            ...
            <Grid size={{ xs: 12, sm: 3 }}>
              <TextField type="date" label="Date" fullWidth size="small" value={filterDate} onChange={e => { const val = e.target.value; setFilterDate(val); load(val || undefined); }} slotProps={{ inputLabel: { shrink: true } }}/>
            </Grid>
            ...
            <Grid size={{ xs: 12, sm: 'auto' }}>
              <Button variant="outlined" size="small" startIcon={<ClearIcon />} onClick={() => {
      ...
      <Card sx={{ ...cardSx(dark) }}>
        <CardContent sx={{ p: 0, '&:last-child': { pb: 0 } }}>
          <Box sx={{ overflowX: 'auto' }}>
            {loading ? (<Box sx={{ p: 3 }}>
                {[...Array(6)].map((_, i) => (<Skeleton key={i} height={50} sx={{ mb: 0.5, borderRadius: 1 }}/>))}
              </Box>) : (<Table>
                <TableHead>
                  <TableRow>
                    {/* ── 8 columns now (added Time) ── */}
                    {['Student Name', 'LRN', 'Gender', 'Date', 'Time', 'Session', 'Status', 'Actions'].map(h => (<TableCell key={h}>{h}</TableCell>))}
                  </TableRow>
                </TableHead>
                <TableBody>
                  {displayed.length === 0 ? (<TableRow>
                    ...
                    </TableRow>) : displayed.map(r => (<TableRow key={r.id}>

                      {/* Student Name */}
                      <TableCell>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                          <StudentAvatar studentName={r.student_name} dark={dark} photoCache={photoCache} onPhotoClick={(name, url) => setPhotoDialog({ name, url })}/>
                    ...
                    fontVariantNumeric: 'tabular-nums',
                      ...
                      {/* Session */}
                      <TableCell>
                        <Chip label={r.session} size="small" sx={{
                    ...
                    border: `1px solid ${dark ? 'rgba(56,197,134,0.25)' : 'rgba(45,80,22,0.2)'}`,
                      ...
                      {/* Status */}
                      <TableCell>
                        <Chip label={canonicalStatus(r.status)} color={chipColor(r.status)} size="small" sx={{ fontWeight: 700, fontSize: '0.7rem' }}/>
                      </TableCell>
                      ...
                      <TableCell>
                        <Box sx={{ display: 'flex', gap: 0.5 }}>
                          ...
                          {r.qr_data ? (<Tooltip title="View & Download QR Code">
                              <IconButton size="small" onClick={() => openQrDialog(r)} sx={iconBtnSx('#6366f1', 'rgba(99,102,241,0.1)')}>
                                <QrCode2Icon sx={{ fontSize: 15 }}/>
                              </IconButton>
                            </Tooltip>) : (<Tooltip title="No QR data for this record">
                              <span>
          ...
          {!loading && displayed.length > 0 && (<Box sx={{ px: 3, py: 1.5, borderTop: `1px solid ${dark ? '#334155' : '#f0f0f0'}` }}>
        ...
        <CardContent sx={{ p: 3 }}>
          ...
          <Box sx={{
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'flex-end' }}>
              <TextField select label="Filter by Status" size="small" value={csvStatusFilter} onChange={e => setCsvStatusFilter(e.target.value)} sx={{ minWidth: 160 }}>
                {csvStatusOptions.map(o => (<MenuItem key={o.value} value={o.value}>{o.label}</MenuItem>))}
              </TextField>

              <Button variant="contained" startIcon={<FileDownloadIcon />} onClick={handleExportCSV} sx={{
            px: 3,
            ...
            {csvMsg && (<Alert severity="success" icon={<CheckCircleIcon fontSize="small"/>} onClose={() => setCsvMsg('')} sx={{ mt: 1.5, borderRadius: '8px', py: 0.5 }}>
                {csvMsg}
              </Alert>)}
            {csvErr && (<Alert severity="error" onClose={() => setCsvErr('')} sx={{ mt: 1.5, borderRadius: '8px', py: 0.5 }}>
                {csvErr}
              </Alert>)}
          </Box>
          ...
          <Box sx={{
            ...
            <Box sx={{ display: 'flex', gap: 2, mb: 2, flexWrap: 'wrap' }}>
              <TextField select label="Month" size="small" value={sf2Month} onChange={e => setSf2Month(e.target.value)} sx={{ minWidth: 140 }}>
                {monthOptions.map(o => (<MenuItem key={o.value} value={o.value}>{o.label}</MenuItem>))}
              </TextField>

              <TextField select label="Year" size="small" value={sf2Year} onChange={e => setSf2Year(e.target.value)} sx={{ minWidth: 110 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              {/* Hidden file input */}
              <input ref={sf2FileRef} type="file" accept=".xlsx,.xls" style={{ display: 'none' }} onChange={handleSf2FileChange} id="sf2-template-input"/>

              {/* Upload button */}
              <Button component="label" htmlFor="sf2-template-input" variant="outlined" startIcon={<UploadFileIcon />} sx={{
            borderRadius: '10px',
            fontWeight: 600,
            ...
            '&:hover': {
              ...
              {/* Chosen file chip */}
              {sf2TemplateFile && (<Chip label={sf2TemplateFile.name} size="small" icon={<CheckCircleIcon style={{ color: dark ? '#4ade80' : '#15803d' }}/>} onDelete={() => {
                setSf2TemplateFile(null);
                if (sf2FileRef.current)
                    sf2FileRef.current.value = '';
            }} sx={{
              ...
              {/* Generate button */}
              <Button variant="contained" startIcon={sf2Loading
            ? <CircularProgress size={15} color="inherit"/>
            : <InsertDriveFileIcon />} disabled={!sf2TemplateFile || sf2Loading} onClick={handleGenerateSF2} sx={{
            px: 3,
            borderRadius: '10px',
            fontWeight: 700,
            background: dark
            ...
            {sf2Msg && (<Alert severity="success" icon={<CheckCircleIcon fontSize="small"/>} onClose={() => setSf2Msg('')} sx={{ mt: 1.5, borderRadius: '8px', py: 0.5 }}>
                {sf2Msg}
              </Alert>)}
            {sf2Err && (<Alert severity="error" onClose={() => setSf2Err('')} sx={{ mt: 1.5, borderRadius: '8px', py: 0.5 }}>
                {sf2Err}
              </Alert>)}
          </Box>
      ...
      {/* ════ Photo Expand Dialog ════ */}
      <Dialog open={photoDialog !== null} onClose={() => setPhotoDialog(null)} maxWidth="sm" fullWidth PaperProps={{
            sx: {
                borderRadius: '16px',
        ...
        <DialogContent sx={{ p: 0 }}>
          {photoDialog && (<Box component="img" src={photoDialog.url} alt={photoDialog.name} sx={{
                width: '100%',
                maxHeight: '70vh',
                objectFit: 'contain',
                display: 'block',
      ...
      {/* ════ QR Code Dialog ════ */}
      <Dialog open={qrDialog !== null} onClose={() => setQrDialog(null)} maxWidth="xs" fullWidth PaperProps={{
            sx: {
                borderRadius: '18px',
                background: dark ? '#0f172a' : '#ffffff',
                overflow: 'hidden',
        ...
        <DialogContent sx={{ pt: 2, pb: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          ...
          {qrDialog && (<Box sx={{
              ...
              <Box component="img" src={getQrImageUrl(qrDialog.qrData, 260)} alt={`QR Code for ${qrDialog.name}`} sx={{ display: 'block', width: 260, height: 260 }}/>
            </Box>)}
        ...
        <DialogActions sx={{ px: 3, py: 2.5, gap: 1 }}>
          <Button onClick={() => setQrDialog(null)} sx={{ color: dark ? '#94a3b8' : '#0f172a' }}>
            Close
          </Button>
          <Button variant="contained" onClick={handleQrDownload} disabled={qrDownloading} startIcon={qrDownloading
            ? <CircularProgress size={15} color="inherit"/>
            : <DownloadIcon />} sx={{
            px: 3,
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
          ...
          <Button variant="contained" onClick={save} disabled={saving || !form.student_name.trim()} startIcon={saving ? <CircularProgress size={15} color="inherit"/> : <SaveIcon />} sx={{
            px: 3,
      ...
      <Dialog open={deleteId !== null} onClose={() => setDeleteId(null)} maxWidth="xs" fullWidth>
        ...
        <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
          ...
          <Button color="error" variant="contained" onClick={confirmDelete} sx={{ background: 'linear-gradient(135deg,#e63946,#c62828)', '&:hover': { background: '#b91c1c' } }}>
            Delete
          </Button>
        </DialogActions>
      </Dialog>
