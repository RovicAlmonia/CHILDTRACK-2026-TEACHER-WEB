...
import { Box, Grid, TextField, Button, Typography, Alert, CircularProgress, MenuItem, LinearProgress, InputAdornment, IconButton, } from '@mui/material';
...
function pwStrength(pw) {
    if (!pw)
        return { pct: 0, label: '', color: 'inherit' };
    if (pw.length < 6)
        return { pct: 20, label: 'Too short', color: 'error' };
    if (pw.length < 8)
        return { pct: 45, label: 'Weak', color: 'error' };
    const hasNum = /[0-9]/.test(pw);
    const hasLet = /[a-zA-Z]/.test(pw);
    const hasSym = /[^a-zA-Z0-9]/.test(pw);
    if (pw.length >= 10 && hasNum && hasLet && hasSym)
        return { pct: 100, label: 'Strong', color: 'success' };
    if (hasNum && hasLet)
        return { pct: 70, label: 'Medium', color: 'warning' };
    return { pct: 45, label: 'Weak', color: 'error' };
}
/* ── Section divider heading ── */
function SectionHead({ label, dark }) {
    const divColor = dark ? 'rgba(255,255,255,0.10)' : 'rgba(0,0,0,0.10)';
    return (<Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 2.5, mb: 1.5 }}>
...
export default function RegisterPage() {
    ...
    return (<Grid container component="main" sx={{
      ...
      {/* ══════════════════════════════════
            LEFT PANEL
        ══════════════════════════════════ */}
      <Grid item xs={false} md={5} lg={5.5} xl={6} sx={{
        ...
        {/* Logo */}
        <Box component="img" src={OverlayImage} alt="CHILDTrack" sx={{
            width: '60%',
            maxWidth: 300,
            objectFit: 'contain',
            position: 'relative',
      ...
      {/* ══════════════════════════════════
            RIGHT PANEL
        ══════════════════════════════════ */}
      <Grid item xs={12} md={7} lg={6.5} xl={6} sx={{
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            overflowY: 'auto',
        ...
        <Box sx={{
          ...
          <Box sx={{
            ...
            const isActive = i === 1;
            return (<Box key={label} component={isActive ? 'div' : Link} to={to ?? undefined} sx={{
          ...
          {/* Alerts */}
          {error && (<Alert severity="error" onClose={() => setError('')} sx={{
          ...
          <Box component="form" onSubmit={handleSubmit}>
            ...
            <Grid container spacing={1.5} sx={{ mb: 0 }}>
              <Grid item xs={12} sm={6}>
                <TextField label="Full Name *" fullWidth size="small" autoFocus value={form.name} onChange={set('name')} sx={fieldSx}/>
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField label="Username *" fullWidth size="small" value={form.username} onChange={set('username')} helperText={<span style={{ color: subtextColor }}>Used to log in</span>} sx={fieldSx}/>
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField label="Password *" type={showPw ? 'text' : 'password'} fullWidth size="small" value={form.password} onChange={set('password')} sx={fieldSx} InputProps={{
            endAdornment: (<InputAdornment position="end">
                ...
                {form.password && (<Box sx={{ mt: 0.6, px: 0.25 }}>
                    <LinearProgress variant="determinate" value={pw.pct} color={pw.color} sx={{
              ...
              <Grid item xs={12} sm={6}>
                <TextField label="Confirm Password *" type={showCf ? 'text' : 'password'} fullWidth size="small" value={form.confirm} onChange={set('confirm')} sx={fieldSx} error={!!form.confirm && form.confirm !== form.password} helperText={form.confirm && form.confirm !== form.password
            ...
            <Grid container spacing={1.5}>
              <Grid item xs={12} sm={6}>
                <TextField label="Age" fullWidth size="small" type="number" value={form.age} onChange={set('age')} inputProps={{ min: 18, max: 100 }} sx={fieldSx}/>
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField label="Gender" fullWidth size="small" select value={form.gender} onChange={set('gender')} sx={fieldSx} SelectProps={{ MenuProps: selectMenuProps }}>
            ...
            <Grid container spacing={1.5}>
              <Grid item xs={12}>
                <TextField label="Section / Grade *" fullWidth size="small" value={form.section} onChange={set('section')} placeholder="e.g. Grade 6 – Section A" sx={fieldSx}/>
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField label="Contact Number" fullWidth size="small" value={form.contact} onChange={set('contact')} placeholder="e.g. 09123456789" sx={fieldSx}/>
              </Grid>
              <Grid item xs={12} sm={6}>
                <TextField label="Address" fullWidth size="small" value={form.address} onChange={set('address')} placeholder="City, Province" sx={fieldSx}/>
              </Grid>
            </Grid>
          ...
          <Box sx={{ mt: 2.5, pt: 2.5, borderTop: `1px solid ${dividerColor}`, textAlign: 'center' }}>
            <Typography sx={{ fontFamily: '"Nunito", sans-serif', fontSize: 12.5, color: subtextColor }}>
              ...
              <Box component={Link} to="/login" sx={{ color: primary, fontWeight: 800, textDecoration: 'none', '&:hover': { textDecoration: 'underline' } }}>
                Sign in
              </Box>
            </Typography>
