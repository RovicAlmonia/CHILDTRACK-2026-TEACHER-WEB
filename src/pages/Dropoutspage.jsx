...
import { Box, Grid, Card, CardContent, Typography, TextField, Button, MenuItem, Table, TableHead, TableRow, TableCell, TableBody, Skeleton, InputAdornment, Avatar, Alert, CircularProgress, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Tooltip, } from '@mui/material';
...
function StudentAvatar({ studentName, photoCache, onPhotoClick }) {
    ...
    return (<Avatar sx={{ ...baseSx, bgcolor: 'action.selected', color: 'text.secondary' }}>
      {initial}
    </Avatar>);
}
/* ─────────── Page helpers ─────────── */
...
export default function DropoutsPage() {
    ...
    const load = (date) => {
        ...
        attendanceAPI.getAll(date || undefined)
            .then(r => {
            const dropouts = r.data.filter((x) => isDroppedOut(x.status));
            setAll(dropouts);
        })
    ...
    return (<Box>
      ...
      <Grid container spacing={2.5} sx={{ mb: 3 }}>
        {statCards.map(s => (<Grid size={{ xs: 6, sm: 3 }} key={s.label}>
        ...
        <CardContent sx={{ p: 0, '&:last-child': { pb: 0 } }}>
          <Box sx={{ overflowX: 'auto' }}>
            {loading ? (<Box sx={{ p: 3 }}>
                ...
                <TableHead>
                  <TableRow>
                    {['Student Name', 'LRN', 'Gender', 'Date Dropped', 'Session', 'Actions'].map(h => (<TableCell key={h}>{h}</TableCell>))}
                  </TableRow>
                </TableHead>
                <TableBody>
