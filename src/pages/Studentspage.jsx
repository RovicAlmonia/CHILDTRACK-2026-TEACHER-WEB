...
import { Box, Grid, Card, CardContent, Typography, TextField, Button, Table, TableHead, TableRow, TableCell, TableBody, Chip, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Alert, CircularProgress, Skeleton, InputAdornment, Avatar, Divider, MenuItem, Tooltip, useTheme, } from '@mui/material';
...
// ── Helpers ──────────────────────────────────────────────────────────────────
const makeEmptyStudent = (section = '') => ({ lrn: '', name: '', age: '', gender: 'Male', contact: '', address: '', section });
...
function buildQRPayload(pg, studentName, studentLrn, studentGender, allGuardians) {
...
function normaliseGuardians(raw) {
    let parsed = [];
    try {
...
function ActivateParentButton({ studentId, initialActive, onToggle, }) {
    ...
    return (<Tooltip title={loading
            ? 'Updating…'
            : isActive
                ? 'Parent mobile account is ACTIVE — click to deactivate'
                : 'Parent mobile account is INACTIVE — click to activate'} arrow placement="top">
      <IconButton size="small" onClick={handleToggle} disabled={loading} sx={{
            border: '1px solid',
        ...
        {loading ? (<CircularProgress size={13} sx={{ color: 'inherit' }}/>) : isActive ? (<LockOpenIcon sx={{ fontSize: 15 }}/>) : (<LockIcon sx={{ fontSize: 15 }}/>)}
      </IconButton>
    </Tooltip>);
}
// ── Single QR card ────────────────────────────────────────────────────────────
...
function GuardianQRCard({ pg, studentName, studentLrn, studentGender, allGuardians, }) {
    const theme = useTheme();
    const isDark = theme.palette.mode === 'dark';
    ...
    return (<Box sx={{
      ...
      <Box sx={{
        ...
        <Box sx={{
          ...
          <img src={qrUrl} alt={`QR for ${pg.role}`} onLoad={() => setImgLoaded(true)} style={{
        ...
        <Box sx={{ textAlign: 'center', width: '100%' }}>
          ...
          {pg.contact_number ? (<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0.6 }}>
            ...
            </Box>) : (<Typography sx={{ fontSize: '0.75rem', color: emptyColor, fontFamily: FONT, fontStyle: 'italic' }}>
              No contact
            </Typography>)}
        </Box>

        {/* Download button */}
        <Box component="button" onClick={handleDownload} disabled={downloading} sx={{
...
function GuardianQRModal({ open, onClose, studentName, studentLrn, studentGender, guardians, }) {
    const theme = useTheme();
    const isDark = theme.palette.mode === 'dark';
    ...
    return (<Dialog open={open} onClose={onClose} maxWidth="md" fullWidth PaperProps={{
      ...
      <DialogContent sx={{ p: 3, background: contentBg }}>
        {namedGuardians.length === 0 ? (<Box sx={{
            ...
            {namedGuardians.map((pg, i) => (<Grid key={i} size={{ xs: 12, sm: 4 }}>
                {/*
                  ↓ pg      = this guardian's own data (name, role, contact)
                  ↓ allGuardians = full 3-slot array so contacts[] stays complete
                */}
                <GuardianQRCard pg={pg} studentName={studentName} studentLrn={studentLrn} studentGender={studentGender} allGuardians={guardians}/>
              </Grid>))}
          </Grid>)}
      </DialogContent>
...
export default function StudentsPage() {
    ...
    const [guardians, setGuardians] = useState(DEFAULT_GUARDIANS.map(g => ({ ...g })));
    useEffect(() => {
        if (teacherSection)
            setForm(p => ({ ...p, section: p.section || teacherSection }));
    }, [teacherSection]);
    const setF = (k) => (e) => setForm(p => ({ ...p, [k]: e.target.value }));
    const updateGuardian = (i, k, v) => setGuardians(gs => gs.map((g, idx) => idx === i ? { ...g, [k]: v } : g));
    const load = () => {
        studentsAPI.getAll()
    ...
    const displayed = rows.filter(r => !search || [r.name, r.lrn].some(v => v?.toLowerCase().includes(search.toLowerCase())));
    return (<Box>
      {/* ── Page header ── */}
      <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} onClick={openAdd} sx={{
        ...
        <CardContent sx={{ p: 2 }}>
          <Grid container spacing={2} sx={{ alignItems: 'center' }}>
            ...
            <Grid size={{ xs: 12, sm: 5 }}>
              <TextField placeholder="Search by name or LRN…" fullWidth size="small" value={search} onChange={e => setSearch(e.target.value)} sx={{ '& input': { fontFamily: FONT } }} slotProps={{
            input: {
            ...
            <Grid size={{ xs: 12, sm: 'auto' }}>
              <Button variant="outlined" size="small" startIcon={<ClearIcon />} onClick={() => setSearch('')} sx={{ fontFamily: FONT, borderColor: 'divider', color: 'text.secondary' }}>
                Clear
              </Button>
            </Grid>
          </Grid>
        ...
        <CardContent sx={{ p: 0, '&:last-child': { pb: 0 } }}>
          <Box sx={{ overflowX: 'auto' }}>
            {loading ? (<Box sx={{ p: 3 }}>
                ...
                <TableHead>
                  <TableRow>
                    {['Student Name', 'LRN', 'Gender', 'Section', 'Parents / Guardians', 'Actions'].map(h => (<TableCell key={h} sx={{ fontFamily: FONT, fontWeight: 700 }}>{h}</TableCell>))}
                  </TableRow>
                </TableHead>
                <TableBody>
                  {displayed.length === 0 ? (<TableRow>
                    ...
                    </TableRow>) : displayed.map(r => {
                const pgs = parsePGs(r.parents_guardians).filter((p) => p.name);
                ...
                return (<TableRow key={r.id}>
                        ...
                        <TableCell>
                          <Chip label={isMale ? 'Male' : 'Female'} size="small" sx={{
                        fontFamily: FONT,
                        bgcolor: isMale ? '#eff6ff' : '#fdf2f8',
                        color: isMale ? '#3b82f6' : '#ec4899',
                        ...
                        <TableCell sx={{ fontFamily: FONT, fontSize: '0.82rem', color: 'text.secondary' }}>
                          {r.section || (<Typography component="span" sx={{ fontFamily: FONT, color: 'text.disabled', fontStyle: 'italic', fontSize: '0.82rem' }}>
                              —
                            </Typography>)}
                        </TableCell>
                        <TableCell>
                          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                            {pgs.length > 0 ? pgs.map((p, i) => {
                        const m = ROLE_META[p.role] || ROLE_META['Parent 1'];
                        return (<Chip key={i} label={p.name} size="small" sx={{
                        ...
                        <TableCell>
                          <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'center' }}>
                            <IconButton size="small" title="View details" onClick={() => setDetailRow(r)} sx={{ color: 'text.secondary', border: '1px solid', borderColor: 'divider', '&:hover': { bgcolor: 'action.hover' } }}>
                              <InfoIcon sx={{ fontSize: 16 }}/>
                            </IconButton>
                            <IconButton size="small" title="View Guardian QR Codes" onClick={() => setQrRow(r)} sx={{
                        ...
                        '&:hover': {
                            ...
                            <ActivateParentButton studentId={r.id} initialActive={Boolean(r.parent_is_active)} onToggle={load}/>
                          </Box>
                        </TableCell>
                      </TableRow>);
            })}
          ...
          {!loading && displayed.length > 0 && (<Box sx={{
                px: 3, py: 1.5,
      ...
      {/* ══ Add Student Dialog ══════════════════════════════════════════════ */}
      <Dialog open={addOpen} onClose={() => setAddOpen(false)} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: '12px', maxHeight: '92vh' } }}>
          ...
          <Grid container spacing={5}>
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              <Grid container spacing={2} sx={{ mb: 2 }}>
                <Grid size={{ xs: 6 }}>
                  ...
                  <TextField fullWidth type="number" placeholder="e.g. 10" value={form.age} onChange={setF('age')} inputProps={{ min: 3, max: 25 }} sx={inputSx}/>
                </Grid>
                <Grid size={{ xs: 6 }}>
              ...
              <Box sx={{ mb: 2 }}>
                ...
                <TextField fullWidth placeholder="e.g. 09123456789" value={form.contact} onChange={setF('contact')} sx={inputSx}/>
              </Box>

              <Box sx={{ mb: 2 }}>
                <Typography component="label" sx={labelSx}>Address</Typography>
                <TextField fullWidth placeholder="Enter student's address" value={form.address} onChange={setF('address')} sx={inputSx}/>
              </Box>
            ...
            return (<Box key={g.role} sx={{
                    ...
                    <Grid container spacing={1.5}>
                      <Grid size={{ xs: 7 }}>
                        ...
                        <TextField size="small" fullWidth placeholder={`${g.role} full name`} value={g.name} onChange={e => updateGuardian(i, 'name', e.target.value)} sx={inputSx}/>
                      </Grid>
                      <Grid size={{ xs: 5 }}>
                        <Typography component="label" sx={{ ...labelSx, fontSize: '12px' }}>Contact</Typography>
                        <TextField size="small" fullWidth placeholder="09XXXXXXXXX" value={g.contact_number} onChange={e => updateGuardian(i, 'contact_number', e.target.value)} sx={inputSx}/>
                      </Grid>
                    </Grid>
                  </Box>);
        })}
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              <Box sx={{ mb: 2 }}>
                ...
                <TextField fullWidth autoFocus placeholder="Juan Dela Cruz" value={form.name} onChange={setF('name')} sx={inputSx}/>
              </Box>
              <Box sx={{ mb: 2 }}>
                <Typography component="label" sx={labelSx}>LRN *</Typography>
                <TextField fullWidth placeholder="Learner Reference Number" value={form.lrn} onChange={setF('lrn')} sx={inputSx}/>
              </Box>

              <Box sx={{ mb: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8, mb: 0.8 }}>
                  <Typography component="label" sx={{ ...labelSx, mb: 0 }}>Section & Grade</Typography>
                  {teacherSection && (<Chip icon={<LockIcon sx={{ fontSize: '10px !important' }}/>} label="auto-filled" size="small" sx={{
                ...
                <TextField fullWidth placeholder="e.g. Grade 6 - A" value={form.section} onChange={setF('section')} sx={inputSx} slotProps={teacherSection ? {
            input: {
        ...
        <DialogActions sx={{ px: 5, pb: 4, pt: 2.5, flexDirection: 'column', gap: 1.5 }}>
          <Button variant="contained" onClick={save} fullWidth disabled={saving || !form.lrn.trim() || !form.name.trim()} startIcon={saving ? <CircularProgress size={16} color="inherit"/> : <SaveIcon />} sx={{
          ...
          <Button onClick={() => setAddOpen(false)} fullWidth sx={{ fontFamily: FONT, py: 1, color: 'text.secondary', '&:hover': { bgcolor: 'action.hover' } }}>
            Cancel
          </Button>
        </DialogActions>
      </Dialog>
      ...
      {/* ── Detail Dialog ── */}
      {detailRow && (<Dialog open onClose={() => setDetailRow(null)} maxWidth="xs" fullWidth>
          <DialogTitle sx={{
                fontFamily: FONT, fontWeight: 700,
                color: (theme) => theme.palette.mode === 'dark' ? '#4ade80' : '#16a34a',
            }}>
          ...
          <DialogContent>
            ...
            ].filter(f => f.value).map((f, i, arr) => (<Box key={f.label} sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  <Box>
            ...
            {parsePGs(detailRow.parents_guardians).filter((p) => p.name).length === 0 ? (<Typography sx={{ fontFamily: FONT, color: 'text.disabled', fontStyle: 'italic', fontSize: '0.88rem' }}>
                No parents or guardians registered.
              </Typography>) : parsePGs(detailRow.parents_guardians).filter((p) => p.name).map((p, i) => {
                const m = ROLE_META[p.role] || ROLE_META['Parent 1'];
                return (<Box key={i} sx={{
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <Chip label={`${m.icon} ${p.role}`} size="small" sx={{
                        fontFamily: FONT,
                        bgcolor: (theme) => theme.palette.mode === 'dark' ? m.badgeDark : m.badgeLight,
                        color: (theme) => theme.palette.mode === 'dark' ? m.accentDark : m.accentLight,
                  ...
                  {p.contact_number && (<Typography sx={{ fontFamily: FONT, fontSize: '0.8rem', color: 'text.secondary', mt: 0.5 }}>📞 {p.contact_number}</Typography>)}
                </Box>);
            })}
          </DialogContent>
          <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
            <Button onClick={() => { setQrRow(detailRow); setDetailRow(null); }} startIcon={<QrCode2Icon />} variant="outlined" sx={{
                fontFamily: FONT,
            ...
            <Button onClick={() => setDetailRow(null)} variant="outlined" sx={{ fontFamily: FONT, color: 'text.secondary' }}>
              Close
            </Button>
          </DialogActions>
        </Dialog>)}
      ...
      {qrRow && (() => {
            ...
            const allPGs = normaliseGuardians(qrRow.parents_guardians);
            return (<GuardianQRModal open onClose={() => setQrRow(null)} studentName={qrRow.name} studentLrn={qrRow.lrn} studentGender={qrRow.gender ?? 'Male'} guardians={allPGs} // full 3-slot array — modal filters named ones for cards
            />);
        })()}
    </Box>);
}
