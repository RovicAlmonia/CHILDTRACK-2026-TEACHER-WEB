...
import {
  ...
  Chip, Skeleton, InputAdornment, Avatar, Alert, CircularProgress,
...
import { attendanceAPI, scanPhotosAPI, absenceReasonsAPI, type AbsenceReason } from '../api';
...
function StudentAvatar({ studentName, photoCache, onPhotoClick }: StudentAvatarProps) {
  ...
  return (
    <Avatar sx={{ ...baseSx, bgcolor: 'rgba(230,57,70,0.12)', color: '#e63946' }}>
      {initial}
    </Avatar>
  );
}
...
const reasonStatusColor = (s: string) => {
  if (s === 'approved') return { color: '#16a34a', bg: 'rgba(22,163,74,0.10)', border: 'rgba(22,163,74,0.25)' };
  if (s === 'rejected') return { color: '#e63946', bg: 'rgba(230,57,70,0.10)', border: 'rgba(230,57,70,0.25)' };
...
export default function AbsencesPage() {
  ...
  const load = (date?: string) => {
    ...
    attendanceAPI.getAll(date || undefined)
      .then(r => setAll(r.data.filter((x: any) => isAbsent(x.status))))
      .catch(() => setError('Failed to load absences.'))
      .finally(() => setLoading(false));
  };

  const loadReasons = (params?: { date?: string; status?: string }) => {
  ...
  useEffect(() => { if (activeTab === 1) loadReasons(); }, [activeTab]);
  ...
  const handleDateChange = (date: string) => { setFilterDate(date); load(date || undefined); };
  const handleClear = () => { setFilterDate(''); setSearch(''); load(undefined); };

  /* ── Reasons tab handlers ── */
  const updateReasonStatus = async (id: number, status: string) => {
    setUpdatingId(id);
    setReasonsError('');
    try {
  ...
  const handleReasonClear = () => {
    setReasonSearch(''); setReasonDate(''); setReasonStatus('');
    loadReasons();
  };
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2.5} sx={{ mb: 3 }}>
        {statCards.map(s => (
          <Grid size={{ xs: 12, sm: 3 }} key={s.label}>
              ...
              <Box sx={{ overflowX: 'auto' }}>
                {loading ? (
                  ...
                  <Table>
                    <TableHead>
                      <TableRow>
                        {['Student Name', 'LRN', 'Gender', 'Date', 'Session', 'Status', 'Actions'].map(h => (
        ...
        <>
              ...
              <Grid container spacing={2} sx={{ alignItems: 'center' }}>
                ...
                <Grid size={{ xs: 12, sm: 2 }}>
                  <TextField
                    ...
                    value={reasonStatus} onChange={e => setReasonStatus(e.target.value)}
                  >
                ...
                <Grid size={{ xs: 12, sm: 4 }}>
                  <TextField
                    placeholder="Search student or submitted by…" fullWidth size="small"
                    value={reasonSearch} onChange={e => setReasonSearch(e.target.value)}
                    slotProps={{
                      input: {
                        startAdornment: (
                          <InputAdornment position="start">
                ...
                <Grid size={{ xs: 6, sm: 'auto' }}>
                  <Button variant="contained" size="small" onClick={handleReasonFilter}
          ...
          <Card sx={{ borderRadius: '14px' }}>
            <CardContent sx={{ p: 0, '&:last-child': { pb: 0 } }}>
              <Box sx={{ overflowX: 'auto' }}>
                {reasonsLoading ? (
                  ...
                  <Table>
                    <TableHead>
                      <TableRow>
                        {['Student Name', 'LRN', 'Reason', 'Date', 'Submitted By', 'Contact', 'Status', 'Actions'].map(h => (
                                  ...
                                  <Tooltip title="Approve">
                                    <span>
                                      <IconButton
                                        ...
                                        onClick={() => updateReasonStatus(r.id, 'approved')}
                                        sx={{
                                          color: '#16a34a',
                                  ...
                                  <Tooltip title="Reject">
                                    <span>
                                      <IconButton
                                        size="small"
                                        disabled={updatingId === r.id}
                                        onClick={() => updateReasonStatus(r.id, 'rejected')}
                                        sx={{
                                          color: '#e63946',
              ...
              {!reasonsLoading && filteredReasons.length > 0 && (
                <Box sx={{ px: 3, py: 1.5, borderTop: '1px solid', borderColor: 'divider' }}>
                  <Typography sx={{ fontSize: '0.8rem', color: 'text.secondary' }}>
