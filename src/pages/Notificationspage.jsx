...
import { Box, Grid, Card, CardContent, Typography, TextField, Button, Chip, Skeleton, InputAdornment, Avatar, Alert, MenuItem, Tab, Tabs, Table, TableHead, TableRow, TableCell, TableBody, Dialog, DialogTitle, DialogContent, DialogActions, IconButton, Tooltip, CircularProgress, } from '@mui/material';
...
export default function NotificationsPage() {
    ...
    const [evtView, setEvtView] = useState('table');
    ...
    const loadAttendance = useCallback(() => {
        ...
        attendanceAPI.getAll()
            .then(r => setAttendance(r.data.filter((x) => x.status !== 'Dropped Out')))
    ...
    const displayedAtt = attendance
        ...
        if (!attType)
            return true;
        if (attType === 'DROP-OFF')
            return ['DROP-OFF', 'Drop-Off'].includes(r.status);
        if (attType === 'PICK-UP')
            return ['PICK-UP', 'Pick-Up'].includes(r.status);
    ...
    const saveEvent = async () => {
        if (!form.title.trim()) {
            setEvtError('Title is required.');
            return;
        }
        if (!form.date) {
            setEvtError('Date is required.');
            return;
        }
        if (!form.time) {
            setEvtError('Time is required.');
            return;
        }
        setEvtError('');
        setSaving(true);
    ...
    return (<Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
          <Button variant="outlined" startIcon={<RefreshIcon />} onClick={() => { loadAttendance(); loadEvents(); }} disabled={attLoading && evtLoading} sx={{
          ...
          {tab === 1 && (<Button variant="contained" startIcon={<AddIcon />} onClick={openAdd} sx={{ px: 2.5 }}>
              Add Event
            </Button>)}
        </Box>
      </Box>
      ...
      <Box sx={{ borderBottom: '2px solid', borderColor: 'divider', mb: 3 }}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{
            ...
            '& .Mui-selected': { color: (theme) => theme.palette.mode === 'dark' ? '#4ade80' : '#2d5016' },
            '& .MuiTabs-indicator': {
                background: (theme) => theme.palette.mode === 'dark' ? '#4ade80' : '#2d5016',
          ...
          <Tab label={<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                Attendance Alerts
                {attCounts.total > 0 && (<Chip label={attCounts.total} size="small" color="primary" sx={{ height: 20, fontSize: '0.7rem', fontWeight: 700 }}/>)}
              </Box>}/>
          <Tab label={<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <EventIcon sx={{ fontSize: 18 }}/>
                Events
                {upcomingCount > 0 && (<Chip label={`${upcomingCount} upcoming`} size="small" sx={{ height: 20, fontSize: '0.7rem', fontWeight: 700, bgcolor: '#f0fdf4', color: '#22c55e' }}/>)}
              </Box>}/>
        </Tabs>
      </Box>
      ...
      {tab === 0 && (<Box>
          ...
          <Grid container spacing={2.5} sx={{ mb: 3 }}>
            {[
                { label: 'Total Today', value: attCounts.total, color: (theme) => theme.palette.mode === 'dark' ? '#4ade80' : '#2d5016', icon: <NotificationsIcon /> },
                ...
                <Card sx={{ borderRadius: '16px' }}>
                  <CardContent sx={{ p: 2.5 }}>
                    {attLoading ? <Skeleton height={60}/> : (<Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <CardContent sx={{ p: 2 }}>
              <Grid container spacing={2} sx={{ alignItems: 'center' }}>
                <Grid size={{ xs: 12, sm: 3 }}>
                  <TextField type="date" label="Filter by Date" fullWidth size="small" value={attDate} onChange={e => setAttDate(e.target.value)} slotProps={{ inputLabel: { shrink: true } }}/>
                </Grid>
                <Grid size={{ xs: 12, sm: 3 }}>
                  <TextField select label="Status Type" fullWidth size="small" value={attType} onChange={e => setAttType(e.target.value)}>
                    {ATTENDANCE_FILTERS.map(o => <MenuItem key={o.value} value={o.value}>{o.label}</MenuItem>)}
                  </TextField>
                </Grid>
                <Grid size={{ xs: 12, sm: 4 }}>
                  <TextField placeholder="Search student name…" fullWidth size="small" value={attSearch} onChange={e => setAttSearch(e.target.value)} slotProps={{ input: { startAdornment: <InputAdornment position="start"><SearchIcon fontSize="small" sx={{ color: 'text.disabled' }}/></InputAdornment> } }}/>
                </Grid>
                <Grid size={{ xs: 12, sm: 'auto' }}>
                  <Button variant="outlined" size="small" startIcon={<ClearIcon />} onClick={() => { setAttDate(''); setAttType(''); setAttSearch(''); }} sx={{ borderColor: 'divider', color: 'text.secondary' }}>
                    Clear
                  </Button>
                </Grid>
              </Grid>
          ...
          {/* Notification cards */}
          {attLoading ? (<Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              {[...Array(5)].map((_, i) => <Skeleton key={i} variant="rounded" height={80} sx={{ borderRadius: '12px' }}/>)}
            </Box>) : displayedAtt.length === 0 ? (<Card sx={{ borderRadius: '14px', textAlign: 'center', py: 7, border: '1.5px dashed', borderColor: 'divider', bgcolor: 'transparent' }}>
            ...
            </Card>) : (<Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
      ...
      {tab === 1 && (<Box>
          ...
          <Grid container spacing={2.5} sx={{ mb: 3 }}>
            {[
                ...
                <Card sx={{ borderRadius: '16px' }}>
                  <CardContent sx={{ p: 2.5 }}>
                    {evtLoading ? <Skeleton height={60}/> : (<Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <Avatar sx={{
                        bgcolor: s.color ? `${s.color}15` : 'rgba(45,80,22,0.1)',
                        width: 44, height: 44, flexShrink: 0,
                    }}>
                          <Box sx={{ color: s.color ?? ((theme) => theme.palette.mode === 'dark' ? '#4ade80' : '#2d5016'), display: 'flex' }}>{s.icon}</Box>
                        </Avatar>
                        <Box>
                          <Typography sx={{ fontSize: '0.68rem', fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 0.6 }}>
                            {s.label}
                        ...
                        color: s.color ?? ((theme) => theme.palette.mode === 'dark' ? '#4ade80' : '#2d5016'),
                    }}>
                            {s.value}
                          </Typography>
                        </Box>
          ...
          <Card sx={{ mb: 2.5, borderRadius: '14px' }}>
            <CardContent sx={{ p: 2 }}>
              <Grid container spacing={2} sx={{ alignItems: 'center' }}>
                <Grid size={{ xs: 12, sm: 3 }}>
                  <TextField select label="Event Type" fullWidth size="small" value={evtTypeFilter} onChange={e => setEvtTypeFilter(e.target.value)}>
                    <MenuItem value="">All Types</MenuItem>
                    {EVENT_TYPES.map(t => <MenuItem key={t} value={t}>{t}</MenuItem>)}
                  </TextField>
                </Grid>
                <Grid size={{ xs: 12, sm: 5 }}>
                  <TextField placeholder="Search title, location, description…" fullWidth size="small" value={evtSearch} onChange={e => setEvtSearch(e.target.value)} slotProps={{ input: { startAdornment: <InputAdornment position="start"><SearchIcon fontSize="small" sx={{ color: 'text.disabled' }}/></InputAdornment> } }}/>
                </Grid>
                <Grid size={{ xs: 12, sm: 'auto' }}>
                  <Button variant="outlined" size="small" startIcon={<ClearIcon />} onClick={() => { setEvtTypeFilter(''); setEvtSearch(''); }} sx={{ borderColor: 'divider', color: 'text.secondary' }}>
                    Clear
                  </Button>
                </Grid>
                <Grid size={{ xs: 12, sm: 'auto' }}>
                  <Box sx={{ display: 'flex', border: '1.5px solid', borderColor: 'divider', borderRadius: '10px', overflow: 'hidden' }}>
                    {['table', 'cards'].map(v => (<Button key={v} size="small" onClick={() => setEvtView(v)} sx={{
                    borderRadius: 0, px: 2, textTransform: 'capitalize',
                    bgcolor: evtView === v
                        ? (theme) => theme.palette.mode === 'dark' ? '#4ade80' : '#2d5016'
                        : 'transparent',
                    color: evtView === v ? '#fff' : 'text.secondary',
                    '&:hover': {
                        bgcolor: evtView === v
                            ? (theme) => theme.palette.mode === 'dark' ? '#4ade80' : '#2d5016'
                            : (theme) => theme.palette.mode === 'dark' ? 'rgba(74,222,128,0.08)' : '#f0f7e8',
                    },
          ...
          {/* TABLE VIEW */}
          {evtView === 'table' && (<Card sx={{ borderRadius: '14px' }}>
              <CardContent sx={{ p: 0, '&:last-child': { pb: 0 } }}>
                <Box sx={{ overflowX: 'auto' }}>
                  {evtLoading ? (<Box sx={{ p: 3 }}>{[...Array(5)].map((_, i) => <Skeleton key={i} height={52} sx={{ mb: 0.5 }}/>)}</Box>) : (<Table>
                      <TableHead>
                        <TableRow>
                          {['Title', 'Type', 'Date & Time', 'Location', 'Description', 'Status', 'Actions'].map(h => (<TableCell key={h}>{h}</TableCell>))}
                        </TableRow>
                      </TableHead>
                      <TableBody>
                        {displayedEvt.length === 0 ? (<TableRow>
                        ...
                        return (<TableRow key={evt.id}>
                              ...
                              <TableCell>
                                <Chip label={evt.event_type ?? 'Other'} size="small" sx={{ bgcolor: style.bg, color: style.color, border: `1px solid ${style.border}`, fontWeight: 600, fontSize: '0.72rem' }}/>
                              </TableCell>
                              <TableCell>
                                <Typography sx={{
                              ...
                              <TableCell sx={{ fontSize: '0.85rem', color: 'text.secondary', maxWidth: 140 }}>
                                {evt.location ? (<Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                              ...
                              <TableCell sx={{ maxWidth: 200 }}>
                                {evt.description ? (<Typography sx={{ fontSize: '0.82rem', color: 'text.secondary', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                    {evt.description}
                                  </Typography>) : '—'}
                              </TableCell>
                              <TableCell>
                                <Chip label={upcoming ? 'Upcoming' : 'Past'} size="small" sx={{
                              ...
                              <TableCell>
                                <Box sx={{ display: 'flex', gap: 0.5 }}>
                                  <Tooltip title="Edit event">
                                    <IconButton size="small" onClick={() => openEdit(evt)} sx={{ color: '#f59e0b', bgcolor: '#fffbeb', border: '1px solid #fde68a', '&:hover': { bgcolor: '#fef3c7' } }}>
                                      <EditIcon sx={{ fontSize: 15 }}/>
                                    </IconButton>
                                  </Tooltip>
                                  <Tooltip title="Delete event">
                                    <IconButton size="small" onClick={() => setDeleteId(evt.id)} sx={{ color: '#e63946', bgcolor: '#fef2f2', border: '1px solid #fecaca', '&:hover': { bgcolor: '#fee2e2' } }}>
                ...
                {!evtLoading && displayedEvt.length > 0 && (<Box sx={{ px: 3, py: 1.5, borderTop: '1px solid', borderColor: 'divider' }}>
                    <Typography sx={{ fontSize: '0.8rem', color: 'text.disabled' }}>
          ...
          {/* CARDS VIEW */}
          {evtView === 'cards' && (evtLoading ? (<Grid container spacing={2.5}>
                {[...Array(4)].map((_, i) => (<Grid size={{ xs: 12, sm: 6, md: 4 }} key={i}>
                    <Skeleton variant="rounded" height={200} sx={{ borderRadius: '16px' }}/>
                  </Grid>))}
              </Grid>) : displayedEvt.length === 0 ? (<Card sx={{ borderRadius: '14px', textAlign: 'center', py: 7, border: '1.5px dashed', borderColor: 'divider', bgcolor: 'transparent' }}>
              ...
              </Card>) : (<Grid container spacing={2.5}>
                {displayedEvt.map(evt => {
                    const fmt = formatScheduled(evt.scheduled_at);
                    const style = getEventTypeStyle(evt.event_type);
                    const upcoming = isUpcoming(evt.scheduled_at);
                    return (<Grid size={{ xs: 12, sm: 6, md: 4 }} key={evt.id}>
                      <Card sx={{
                            borderRadius: '16px',
                        ...
                        <CardContent sx={{ p: 2.5, flex: 1 }}>
                          <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 1.5 }}>
                            <Box sx={{ flex: 1, pr: 1 }}>
                              ...
                              <Chip label={evt.event_type ?? 'Other'} size="small" sx={{ bgcolor: style.bg, color: style.color, border: `1px solid ${style.border}`, fontWeight: 600, fontSize: '0.7rem' }}/>
                            </Box>
                            <Chip label={upcoming ? 'Upcoming' : 'Past'} size="small" sx={{
                          ...
                          {evt.location && (<Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 1 }}>
                        ...
                        <Box sx={{ px: 2.5, pb: 2, pt: 0, display: 'flex', gap: 1 }}>
                          <Button size="small" startIcon={<EditIcon />} onClick={() => openEdit(evt)} sx={{ flex: 1, color: '#f59e0b', bgcolor: '#fffbeb', border: '1px solid #fde68a',
                            fontSize: '0.78rem', fontWeight: 600, '&:hover': { bgcolor: '#fef3c7' } }}>
                            Edit
                          </Button>
                          <Button size="small" startIcon={<DeleteIcon />} onClick={() => setDeleteId(evt.id)} sx={{ flex: 1, color: '#e63946', bgcolor: '#fef2f2', border: '1px solid #fecaca',
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ pt: '12px !important' }}>
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField type="time" label="Time *" fullWidth value={form.time} onChange={setF('time')} slotProps={{ inputLabel: { shrink: true } }}/>
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField label="Location" fullWidth value={form.location} onChange={setF('location')} placeholder="e.g. School Auditorium"/>
            </Grid>
            <Grid size={{ xs: 12 }}>
              <TextField label="Description" fullWidth multiline rows={3} value={form.description} onChange={setF('description')} placeholder="Optional details about this event…"/>
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
          <Button onClick={() => setDialogOpen(false)} sx={{ color: 'text.secondary' }}>Cancel</Button>
          <Button variant="contained" onClick={saveEvent} disabled={saving || !form.title.trim() || !form.date || !form.time} startIcon={saving ? <CircularProgress size={16} color="inherit"/> : <SaveIcon />} sx={{ px: 3 }}>
      ...
      <Dialog open={deleteId !== null} onClose={() => setDeleteId(null)} maxWidth="xs" fullWidth>
        ...
        <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
          ...
          <Button color="error" variant="contained" onClick={confirmDelete} sx={{ background: 'linear-gradient(135deg, #e63946, #c62828)' }}>
            Delete
          </Button>
        </DialogActions>
      </Dialog>
